-- Aggregates for the Maps surface at the second level below country (zone).
--
...
--
-- `pcode` joins to level2.geojson. Nothing is called `name`: Evidence's map
-- input proxy is a callable and a `name` field collides with Function.name,
-- which kills the map outright.
select
    f.geo_3_id    as pcode,
    f.geo_3       as area_name,
    f.geo_2_id as parent_pcode,
    count(*)                                                                  as farmers,
