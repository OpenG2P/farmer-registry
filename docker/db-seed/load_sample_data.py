...
def remap_links(table: str, rows: list, remap: dict) -> list:
    ...
    """
    if table in LAND_PARENTED_TABLES:
        # Crop / livestock / farm inputs always point at a land parcel. Do not
        # apply the person/household remap or they would become children of the
        # farmer and disappear from the land-based hierarchy walk.
        _REMAP_STATS[table] = (len(rows), len(rows), len(rows))
        return rows
    if not remap:
        return rows
    ...
    for r in rows:
        ...
        if link in remap:
            ...
            # parcel. Those ids stay as landNNNN; remapping them onto a farmer
            # would break the land parent required by Crop/Livestock/FarmInputs.
            unresolved += 1
        out.append(r)
...
# Crop, livestock and farm inputs are children of land (master_register_id =
# Land). Never attach them to a farmer id — the hierarchy walk is
# farmer → land → crop/livestock/inputs.
LAND_PARENTED_TABLES = {
    "g2p_register_crops",
...
def main() -> None:
    ...
    try:
        ...
        land_ids = {
            r["internal_record_id"]
            for r in remap_links(
                "g2p_register_lands", fixtures["lands.json"], remap
            )
            if r.get("internal_record_id")
        }
        for table, fname, extras, json_cols in SUB_TABLES:
            ...
            if table in LAND_PARENTED_TABLES:
                kept, dropped = [], 0
                for r in rows:
                    if r.get("link_internal_record_id") in land_ids:
                        kept.append(r)
                    else:
                        dropped += 1
                if dropped:
                    print(
                        f"[load-sample-data]   {table}: dropped {dropped} row(s) "
                        "with no parent land"
                    )
                rows = kept
